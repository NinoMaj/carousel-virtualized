...
interface ICarouselVirtualizedProps {
  ...
  onPositionChange({ newIndex, eventName }): void;
...
> {
  ...
  public render() {
    ...
    const calculatedInitialScrollOffset: number = initialScrollOffset
      ? initialScrollOffset
      : (itemSize || this.state.containerWidth) as number * currentIndex;
  ...
  private computeNextSlide() {
    ...
    this.onPositionChange(slidesMoved, this.state.eventName);
  }
  ...
  private handleLeftArrowClick() {
    this.onPositionChange(DIRECTION.LEFT, EVENT_NAME.LEFT_ARROW_CLICK);
  }

  private handleRightArrowClick() {
    this.onPositionChange(DIRECTION.RIGHT, EVENT_NAME.RIGHT_ARROW_CLICK);
  }
