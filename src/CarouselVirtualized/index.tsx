...
type CarouselVirtualizedState = { containerWidth: any, containerHeight: any, isMounted: boolean } & {
  currentIndex: any
} & { isMouseDragActive: any, isTouchDragActive: any, startX: any, startY: any } & ((
    state: any
  ) => { deltaX: number, deltaY: number, mouseIsMoving: any }) & {
    deltaX: number,
    deltaY: number,
    isMouseDragActive: boolean,
    isTouchDragActive: boolean,
    mouseIsMoving: boolean
  } & { currentIndex: any, eventName: any } & {
...
class CarouselVirtualized extends React.PureComponent<CarouselVirtualizedProps, CarouselVirtualizedState> {
  static slidesMoved(deltaX, deltaY, itemSize) {
    ...
    return deltaX < 0 ? Math.max(smallDrag, bigDrag) : -Math.max(bigDrag, smallDrag);
  }
  constructor(props) {
    super(props);
    this.state = {
  ...
  handleOnMouseClick() {
    ...
    console.log(this.state.mouseIsMoving);
  ...
  computeNextSlide() {
    const slidesMoved = CarouselVirtualized.slidesMoved(this.state.deltaX, this.state.deltaY, this.itemSize());
  ...
  itemSize() {
    ...
    return itemSize ? itemSize : slideCount ? width / slideCount : this.state.containerWidth || 0;
  }
  render() {
    const {
      children,
    ...
    const calculatedItemSize = itemSize ? itemSize : slideCount ? width / slideCount : this.state.containerWidth || 0;
    const calculatedHeight = height || this.state.containerHeight || 0;
    const calculatedWidth = itemSize && slideCount ? itemSize * slideCount : width || this.state.containerWidth || 0;
